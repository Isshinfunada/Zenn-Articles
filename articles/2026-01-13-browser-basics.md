...
- **cargo**: Rustのパッケージマネージャー兼ビルドツール。プロジェクトのビルド、テストに使用する
...
macOSではバージョンにより readlink -f の有無が異なる（少なくとも 12.2 までは -f が無いという報告があり、12.3以降で登場）

- **原因**: MacOSはBSD系のOSであり、Linux（GNU系）とは`readlink`コマンドの実装が異なる場合がある。
